(function() {
  ...
  const state = {
    step: 0,
    totalSteps: 9, // Includes new Welcome step (0) and Results
    answers: {},
    claims: [],
    salary: 35000,
    amountOwed: 0,
  ...
  function trackGAEvent(eventName, eventParams) {
    if (typeof window.gtag === 'function') {
      ...
      console.log(`%cGA Event Fired: %c${eventName}`, 'color: #3b82f6; font-weight: bold;', 'color: inherit;', eventParams);
    }
  }
  ...
  function mount() {
    ...
    root.innerHTML = '';
    const styles = `/* ... styles will be injected by buildStyles() ... */`;
    const quizHtml = `
    ...
    `;
    root.append(h(buildStyles()));
    root.append(h(quizHtml));
  ...
  function buildStyles() {
    return `
      <style>
        ...
        .wfq-step { display: none; padding: 24px; background: #fff; border: 1px solid #e5e7eb; border-radius: 12px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); margin-bottom: 16px; animation: fadeIn 0.5s ease-in-out; }
  ...
  function attachListeners() {
    ...
    // Delegated change listener for checkboxes and inputs
    ...
    salaryInput.addEventListener('input', salarySync);

    // Form submission
    const form = document.getElementById('wfq-contact-form');
    form.addEventListener('submit', e => {
      ...
      goToStep(9); // Go to results step
    });
  }

  function goToStep(stepNumber) {
    if (state.step === 0 && stepNumber === 1) {
      trackGAEvent('quiz_started', { event_category: 'engagement' });
    }
    state.step = stepNumber;
    updateView();
    trackGAEvent('step_viewed', { event_category: 'quiz_flow', value: stepNumber });
  }

  function updateView() {
    document.querySelectorAll('.wfq-step').forEach(el => el.classList.remove('active'));
    document.getElementById(`wfq-step-${state.step}`).classList.add('active');

    const progress = state.step >= 8 ? 100 : ((state.step) / (state.totalSteps - 2)) * 100;
    document.getElementById('wfq-progress-fill').style.width = `${progress}%`;

    // Validation logic for continue buttons
  ...
  function calculateResult() {
    ...
    const weeklyPay = Math.min(parseInt(state.salary, 10) / 52, 643); // Using 2023 cap
    const basicAward = weeklyPay * 1.5 * 10; // Simplified example
  ...
  function sendSubmission(result) {
    ...
    // const webhookUrl = 'https://your-attio-webhook-url.com/xyz';
    // fetch(webhookUrl, {
    //   method: 'POST',
    //   headers: { 'Content-Type': 'application/json' },
    //   body: JSON.stringify(payload)
    // }).catch(err => console.error('Submission failed', err));
    trackGAEvent('quiz_completed', { event_category: 'conversion', value: result.compensation });
